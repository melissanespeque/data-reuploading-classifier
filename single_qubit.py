...
def single_qubit_predict_wcf(x, n_layers, n_classes, theta, w, alpha, reprs):
    ...
    for xi in x:
        ...
        scores = np.zeros(n_classes)
        for c in range(n_classes):
            scores[c] += (alpha[c].item()*state_fidelity(sv, Statevector(reprs[c]))).real
...
def weighted_single_cost_function(params,n_layers, n_classes, x, y, reprs):
    ...
    for i in range(len(x)):
        summ = 0
        Y = get_Y(n_classes, y[i], reprs)
        for c in range(n_classes):
            # alpha[c].item() = to have the float inside the 1D array
            summ += (alpha[c].item()*state_fidelity(sv[i], reprs[c]) -Y[c])**2
        wcf += summ
